        ...
        if (!tako.isGrounded) {
          if (tako.hasAirJump) {
            const { vx, vy, facingRight } = calculateMobileJump(
              tako.chargeRatio, jumpDirectionRef.current, 0
            );
          ...
          const { vx, vy, facingRight } = calculateMobileJump(
            tako.chargeRatio, jumpDirectionRef.current, slidingVelocity
          );
          tako.velocity = { x: vx, y: vy };
          tako.state = 'jumping';
  ...
  useEffect(() => {
    ...
    const checkInput = () => {
      ...
      if (touchInput.jumpJustReleased) {
        ...
        else if (state.screen === 'cleared') nextStage();
        else if (state.screen === 'gameover') returnToTitle();
      }
    };

    const intervalId = setInterval(checkInput, 16);
    return () => clearInterval(intervalId);
  }, [state.screen, startGame, nextStage, returnToTitle]);

  // Canvas上のタップ処理（ポーズボタン、ポーズメニュー）
    ...
    <View style={styles.container}>
      <Pressable onPress={handleCanvasPress}>
        ...
        >
          ...
          {(state.screen === 'playing' || state.screen === 'paused') && <HUD state={state} />}
